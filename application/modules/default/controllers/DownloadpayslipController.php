...
class Default_DownloadpayslipController extends Zend_Controller_Action
{
    private $options;
    ...
    {
		$servicedeskdepartmentmodel = new Default_Model_Downloadpayslip();	
		...
		$this->render('downloadpayslip/index', null, true);
	 	...
	 	try
			...
			{                          
			    if(is_numeric($id) && $id>0)
				{
					...
					$data = $editModel->getEmployeeDatabyuserID($id);
                                        if(!empty($data))
					{
		  ...
		  if($empform->isValid($this->_request->getPost())){
			...
			$tableid  = ''; 
			   $data = array('firstname'=>$firstname, 
				...
				{
				   $tableid = $id;
				   $this->_helper->getHelper("FlashMessenger")->addMessage(array("success"=>"Payslip updated successfully."));
				}   
				else
				{
				   $tableid = $Id; 	
					$this->_helper->getHelper("FlashMessenger")->addMessage(array("success"=>"Payslip added successfully."));					   
				}   
