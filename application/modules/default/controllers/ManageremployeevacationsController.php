	...
	public function save($managerleaverequestform,$appliedleavescount,$employeeemail,$employeeid,$userfullname,$from_date,$to_date,$reason,$businessunitid,$leavetypeid,$leavereqdata)
	{
						...
						$Id = $leaverequestmodel->SaveorUpdateLeaveRequest($data, $where);
							...
							/* Mail to Employee */
								...
								if($messagestr == 'Leave request approved.'){
									...
									$options['message'] = '<div>Hello '.$userfullname.',</div><div>The below leave(s) has been approved.</div>';
								}elseif($messagestr == 'Leave request rejected.'){ 
									$options['subject'] = $messagestr;
									$options['message'] = '<div>Hello '.$userfullname.',</div><div>The below leave(s) has been rejected. </div>';
								}else{
									$options['subject'] = $messagestr;
									$options['message'] = '<div>Hello '.$userfullname.',</div><div>The below leave(s) has been cancelled. </div>';
								}	
								$options['message'] .= '<div>
		...
		/* Mail to Reporting Manager */
            if(!empty($repMgrEmail) && !empty($repMgrName)) {                    
								...
								if($messagestr == 'Leave request approved.'){
									$options['subject'] = $messagestr;
									$options['message'] = '<div>Hello '.$repMgrName.',</div><div>The below leave(s) has been approved.</div>';
								}elseif($messagestr == 'Leave request rejected.'){ 
									$options['subject'] = $messagestr;
									$options['message'] = '<div>Hello '.$repMgrName.',</div><div>The below leave(s) has been rejected. </div>';
								}else{
									$options['subject'] = $messagestr;
									$options['message'] = '<div>Hello '.$repMgrName.',</div><div>The below leave(s) has been cancelled. </div>';
								}	
								$options['message'] .= '<div>
                <table width="100%" cellspacing="0" cellpadding="15" border="0" style="border:3px solid #BBBBBB; font-size:16px; font-family:Arial, Helvetica, sans-serif; margin:30px 0 30px 0;" bgcolor="#ffffff">
                      <tbody><tr>
            ...
            <div style="padding:20px 0 10px 0;">Please <a href="'.BASE_URL.'/index" target="_blank" style="color:#b3512f;">click here</a> to login and check the leave details.</div>';	
                                $result = sapp_Global::_sendEmail($options);
            }                    
							/* END */                                
							...
							/* Mail to HR */	
								...
								if($messagestr == 'Leave request approved.'){
									$options['subject'] = $messagestr;
									$options['message'] = '<div>Hello HR,</div><div>The below leave(s) has been approved.</div>';
								}elseif($messagestr == 'Leave request rejected.'){ 
									$options['subject'] = $messagestr;
									$options['message'] = '<div>Hello HR,</div><div>The below leave(s) has been rejected. </div>';
								}else{
									$options['subject'] = $messagestr;
									$options['message'] = '<div>Hello HR,</div><div>The below leave(s) has been cancelled. </div>';
								}	
								$options['message'] .= '<div>
                <table width="100%" cellspacing="0" cellpadding="15" border="0" style="border:3px solid #BBBBBB; font-size:16px; font-family:Arial, Helvetica, sans-serif; margin:30px 0 30px 0;" bgcolor="#ffffff">
                      <tbody>
