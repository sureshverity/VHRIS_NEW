				...
				if($form->isValid($this->_request->getPost()))
				{
					...
					{ 
						...
						try
						{
							...
							$options['header'] = 'Greetings from Verity';
				...
				{  
						...
						else if(!empty($data))
						{
							...
							$employeeform->setDefault('prefix_id',$data['prefix_id']);
                                                        $employeeform->setDefault('biometricid',$data['biometricId']);
                                                        
                                                        $avayaCode = '';
                                                        
                                                        if(isset($data['avayacode']))
                                                        {
                                                            $avayaCode =  base64_decode($data['avayacode']);
                                                        }
                                                        else
                                                            $avayaCode =''; 
                                                        $employeeform->setDefault('avayacode',$avayaCode);
							...
							$this->view->id = $id;
							$this->view->form = $employeeform;
                                                        $this->view->loggedinuser = $loginUserId;
                                                        $this->view->group_id = $loginuserGroup; 
...
public function editappraisal($id,$performanceflag,$ff_flag)
	...
	public function save($employeeform)
	{
		...
		$office_faxnumber = $this->_request->getParam('office_faxnumber',null);
                $biometricid=$this->_request->getParam('biometricid',null);
                $avayacode =$this->_request->getParam('avayacode',null);
...
//                                 print_r($cand_data[id]);
//                                 print_r($cand_data[requisition_id]);
			...
			try
			{
				...
				else
				{
					...
					$user_where = '';
                               ...
                               // $candidate_data = array("cand_status" =>"Recruited");
                                //$cand_where =array('id' =>$cand_data[id] ,'requisition_id'=>$cand_data[requisition_id]);
                    //  print_r($user_data);  exit;
				$user_status = $usersModel->SaveorUpdateUserData($user_data, $user_where);
                                
                                if($cand_data[id])
                                    {
                                        //$cand_status = $candidate_model->SaveorUpdateCandidateData($candidate_data,$cand_where);
                               
                                        $status = "Recruited";
                                        $candid = $cand_data[id];
                                        $reqid = $cand_data[requisition_id];
                                        $cand_status = $candidate_model->updatecandidatestatus($status,$candid,$reqid);
                                    }
                                //  print_r($cand_status); exit;
				...
				$data = array(  'user_id'=>$user_id,
                                ...
                                'avayacode'=>($avayacode!=''?base64_encode($avayacode):NULL),
                                'date_of_joining'=>$date_of_joining,
