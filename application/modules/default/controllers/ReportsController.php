...
class Default_ReportsController extends Zend_Controller_Action
{
	...
	//	$ajaxContext->addActionContext('empscreening', 'html')->initContext();
		...
		$ajaxContext->addActionContext('performancereport', 'html')->initContext();
                $ajaxContext->addActionContext('payrollreport', 'html')->initContext();
                $ajaxContext->addActionContext('getpayrollreportsdata', 'html')->initContext();
                $ajaxContext->addActionContext('attendancereport', 'html')->initContext();
                $ajaxContext->addActionContext('getattendancereportsdata', 'html')->initContext();
	}
	...
	**/
	 ...
	 */
        public function exportpayrollrptAction(){
            $this->_helper->layout->disableLayout();
             $search_arr = array();
		$search_arr['bunit'] = $this->_getParam('bunit',null);
		$search_arr['department'] = $this->_getParam('department',null);
		$search_arr['year'] = $this->_getParam('year',null);
		$search_arr['period'] = $this->_getParam('period',null);
                $page_no = $this->_getParam('page_no',1);
		...
		$sort_type = $this->_getParam('sort_type',null);
                ...
                $cols_param_arr = $this->_getParam('cols_arr',array());
		$payrollreport_model = new Default_Model_Payrollreports();
                $payroll_components = $payrollreport_model->getComponentByPeriod($search_arr);
                
		$payroll_data = $payrollreport_model->getpayrollreportdata($search_arr,$per_page,$page_no,$sort_name,$sort_type);
                $page_cnt = $payroll_data['page_cnt'];
		$user_data = $payroll_data['rows'];
                $arrEmployee = array();
               
                foreach ($payroll_components as $value) {
                   
                    $id = "".$value['id']."";
                    $text = $value['name'];
                    $columns_arr[$id] = $text;
                }
                $columns_arr["earnings"] = "Total Earnings";
                $columns_arr["deductions"] = "Total Deductions";
                $columns_arr["net"] = "Net Salary";
            
                foreach($payroll_data['rows'] as $row)
                {
                    if($arrEmployee)
                    {
                        $isture = $this->searchForValue($row["employeeId"], $arrEmployee,'employeeId');
                        if ($isture > -1) {
                            if($row["Operator"] == "+")
                                {
                                    $totalEarning = $arrEmployee[$isture]["earnings"] + $row["Amount"];
                                    $arrEmployee[$isture]["earnings"] = $totalEarning;
                                    $arrEmployee[$isture][$row["ComponentId"]] = $row["Amount"];
                                    $grossSalary = $arrEmployee[$isture]["net"] + $row["Amount"];
                                    $arrEmployee[$isture]["net"] = $grossSalary;
                                }
                                else
                                {
                                    $totalEarning = $arrEmployee[$isture]["deductions"] + $row["Amount"];
                                    $arrEmployee[$isture]["deductions"] = $totalEarning;
                                    $arrEmployee[$isture][$row["ComponentId"]] = '-'.$row["Amount"];
                                    $grossSalary = $arrEmployee[$isture]["net"] - $row["Amount"];
                                    $arrEmployee[$isture]["net"] = $grossSalary;
                                }
                            ...
                            $arrEmployee[$currentId]["employeeId"] = $row["employeeId"];
                            $arrEmployee[$currentId]["userfullname"] = $row["userfullname"];
                            $arrEmployee[$currentId]["emprole_name"] = $row["emprole_name"];
                            if($row["Operator"] == "+")
                                {
                                    $totalEarning = $arrEmployee[$currentId]["earnings"] + $row["Amount"];
                                    $arrEmployee[$currentId]["earnings"] = $totalEarning;
                                    $arrEmployee[$currentId][$row["ComponentId"]] = $row["Amount"];
                                    $grossSalary = $arrEmployee[$currentId]["net"] + $row["Amount"];
                                    $arrEmployee[$currentId]["net"] = $grossSalary;
                                }
                                else
                                {
                                    $totalEarning = $arrEmployee[$currentId]["deductions"] + $row["Amount"];
                                    
                                    $arrEmployee[$currentId]["deductions"] = $totalEarning;
                                    $arrEmployee[$currentId][$row["ComponentId"]] = '-'.$row["Amount"];
                                    $grossSalary = $arrEmployee[$currentId]["net"] - $row["Amount"];
                                    $arrEmployee[$currentId]["net"] = $grossSalary;
                                }
                    ...
                    {
                        $arrEmployee[0] = array();
                        $arrEmployee[0]["employeeId"] = $row["employeeId"];
                        $arrEmployee[0]["userfullname"] = $row["userfullname"];
                        $arrEmployee[0]["emprole_name"] = $row["emprole_name"];
                        if($row["Operator"] == "+")
                        {
                            $arrEmployee[0]["Earnings"] = $row["Amount"];
                            $arrEmployee[0][$row["ComponentId"]] = $row["Amount"];
                            $arrEmployee[0]["net"] = $row["Amount"];
                        }
                        else
                        {
                            $arrEmployee[0]["Deductions"] = $row["Amount"];
                            $arrEmployee[0][$row["ComponentId"]] = '-'.$row["Amount"];
                            $arrEmployee[0]["net"] = $row["Amount"];
                        }
                    }
		...
		sapp_Global::export_to_excel($arrEmployee,$cols_param_arr,"payrollreport.xlsx");
	...
	public function getactiveuserdataAction()
	{
        ...
        public function  getattendancereportsdataAction()
        {
                $search_arr = array();
		$search_arr['bunit'] = $this->_getParam('bunit',null);
		$search_arr['department'] = $this->_getParam('department',null);
		...
		$sort_type = $this->_getParam('sort_type',null);                
                $cols_param_arr = $this->_getParam('cols_arr',array());
                $attendancereport_model = new Default_Model_Attendancestatuscode();
                $attendancereport = $attendancereport_model->getattendancereportdata($search_arr,$per_page,$page_no,$sort_name,$sort_type);
                $page_cnt = $attendancereport['page_cnt'];
		$user_data = $attendancereport['rows'];
		...
		$this->view->columns_arr = $columns_arr;
            ...
            public function getpayrollreportsdataAction()
	{
                $search_arr = array();
		$search_arr['bunit'] = $this->_getParam('bunit',null);
		$search_arr['department'] = $this->_getParam('department',null);
		...
		if(count($cols_param_arr) == 0)
		$cols_param_arr = $columns_arr;                
		$this->view->user_data = $arrEmployee;
		$this->view->per_page = $per_page;
		$this->view->page_no = $page_no;
		$this->view->sort_name = $sort_name;
		$this->view->sort_type = $sort_type;
	...
	public function getexcelreportholidayAction()
	{
        ...
        public function payrollreportAction()
	{
                $norec_arr = array();
		$form = new Default_Form_Payrollreport();
		$generatepayrollmodel = new Default_Model_Generatepayroll();
		$bunit_data = $generatepayrollmodel->getBusinessunitList();
		if(count($bunit_data) > 0)
		{
			foreach($bunit_data as $res) 
                            $form->bunit ->addMultiOption($res['id'],utf8_encode($res['unitname']));
                        
                       $dept_data = $generatepayrollmodel->getDepartment($bunit_data[0]["id"]);
                       $form->department ->addMultiOption('0','No Department');
                       foreach($dept_data as $res) 
                            $form->department ->addMultiOption($res['id'],utf8_encode($res['deptname']));
                       $years_data = $generatepayrollmodel->getPayrollyears();
                       foreach($years_data as $res) 
                            $form->year ->addMultiOption($res['year'],utf8_encode($res['year']));
                       $payrollperiod = new Default_Model_Payrollperiods();
                        $period_data = $payrollperiod->getPayrollPeriodByYear($years_data[0]['year'],$bunit_data[0]["id"]);
                       foreach($period_data as $res) 
                            $form->period ->addMultiOption($res['id'],utf8_encode($res['name']));
		}
		else
		{
			$norec_arr['bunit'] = "Business Unit are not added yet.";
		}
