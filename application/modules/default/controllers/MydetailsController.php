						...
						{  
							...
							if(!empty($empdata))
							{
							  ...
							  if(!empty($identityDocumentArr))
								...
								$userdata = 	$usersModel->getUserDetails($id);
                                                                
							  if(!empty($userdata))
                                                            {
                                                    $emppersonaldetailsform->setDefault('firstname',$userdata[0]['firstname']);
                                                    $emppersonaldetailsform->setDefault('lastname',$userdata[0]['lastname']);
                                                        } 
							  if(!empty($data))
								{	
			...
			if(in_array('emppersonaldetails',$empOrganizationTabs)){
				...
				else if(!empty($empdata))
				{
					...
					{   
						...
						$racecodemodel = new Default_Model_Racecode();	
						$languagemodel = new Default_Model_Language();
						//$perfixmodel = new Default_Model_Prefix();
                                                $msgarray = array();
						$userdata = $usersModel ->getUserDetails($id);		 
						$genderlistArr = $genderModel->getGenderList();
                                                $emppersonaldetailsform->removeElement('prefix_id');
						if($loginUserRole == 14)
                                                {
...
//                                                            {
//                                                                    $emppersonaldetailsform->prefix_id->addMultiOption($prefixres['id'],$prefixres['prefix']);
//                                                            }
//                                                    }
//                                                    else
//                                                    {
...
//                                                    }
                                                ...
                                                else
                                                {
                                                    ...
                                                    $emppersonaldetailsform->removeElement('firstname');
                                                    $emppersonaldetailsform->removeElement('lastname');
                                                }
				...
				else
				{
					...
					{	
						...
						$tableFields = array('action'=>'Action','educationlevel'=>'Education Level',
										 ...
										 'from_date'=>'From',"to_date"=>"To","percentage"=>"Percentage","remarks"=>"Remarks");
           ...
           {	
			...
			{	
				...
				case 'education':	//Employee education details
						...
						$remarks = $this->_request->getParam('remarks');
