		...
		if($form->isValid($this->_request->getPost()))
		{
			...
			{	
                            ...
                            $data= array(
                                'emprole' =>14,
                                'userstatus'=>'old',
                                'firstname' =>$candidateData[candidate_firstname],
                                'lastname' =>$candidateData[candidate_lastname],
                                'userfullname' =>$candidateData[candidate_name],
                                'emailaddress' =>$candidateData[emailid],
                                'contactnumber' =>$candidateData[contact_number],
                                'backgroundchk_status' =>$candidateData[backgroundchk_status],
                                'emppassword' =>md5('Verity123'),
                                'createdby' =>$candidateData[createdby],
                                'modifiedby' =>$candidateData[modifiedby],
                                'createddate' =>"$candidateData[createddate]",
                                'modifieddate' =>"$candidateData[modifieddate]",
                                'isactive' =>$candidateData[isactive],
                                'employeeId' =>'CND'.mt_rand(10000000, 99999999),
                                'rccandidatename'=>$req_id,
                                'modeofentry' =>'Interview',
                                'selecteddate' =>"$candidateData[createddate]"
                                
                            );
                            $where ='';
                            $userid = '';
                            $email_count = $usersmodel->getEmailAddressCount($candidateData[emailid]);
                           
                            $savecandidate = $usersmodel->addOrUpdateUserModel($data, $where, $userid);
			    ...
			    $requimodel->incrementselected_members($req_id);
                            ...
                            $empdata = array(
                                'user_id' =>$savecandidate,
                                'date_of_joining' =>"$candidateData[createddate]",
                                'reporting_manager' =>$requisition_data[reporting_id],
                                'emp_status_id' =>11,
                                'businessunit_id' =>$requisition_data[businessunit_id],
                                'department_id' =>$requisition_data[department_id],
                                'jobtitle_id' =>$requisition_data[jobtitle],
                                'position_id' =>$requisition_data[position_id],
                                'createdby' =>$requisition_data[createdby],
                                'modifiedby' =>$requisition_data[modifiedby],
                                'createddate' =>"$requisition_data[createdon]",
                                'modifieddate' =>"$requisition_data[modifiedon]",
                                'isactive' =>$requisition_data[isactive],
                            );
                            $employee_model->SaveorUpdateEmployeeData($empdata, $where);
                          
                                $status_update = array("cand_status" =>'Recruited');
                                $where_update = array("emailid" =>$candidateData[emailid]);
                                 $cand_model->SaveorUpdateCandidateData($status_update,$where_update);
                           
                            $report_person_data = $user_model->getUserDataById($requisition_data['reporting_id']);
                           
                           // $this->createCandidateEmployee($cand_data,$requisition_data,$report_person_data);
                            $mail_arr = array(  'HR'=>  defined('REQ_HR_'.$requisition_data['businessunit_id'])?constant('REQ_HR_'.$requisition_data['businessunit_id']):"",
                                                $cand_data[candidate_name] => $cand_data[emailid],
...
//                            if($mail_arr[HR] =='')
//                            {
//                                $mail_arr[HR] ='vangapandu.suresh@verityinfosol.com';
//                            }
                           ...
                           foreach($mail_arr as $ename => $email)
                            {
                                ...
                                if( $email == $cand_data[emailid])
                                {
                                   $URL = "<a href='http://itms.sslverity.com:8080/VHRIS/'>http://itms.sslverity.com:8080/VHRIS/</a>";
                                   $password = 'Verity123';
                                   $text = "Dear ".$cand_data[candidate_name].",<br>";
                                   $text .= "Greetings & Congratulations from Verity Information Solutions you are selected with reference to your interview.<br>";
                                   $text .="I request you to upload below list of documents in the Verity Portal for the offer letter to be released.<br><br>								
								
                                                1. Soft copy of the resume<br>								
                                                2. Scanned passport /Aadhar Card<br>								
                                                3. Scanned PAN card	<br>							
                                                4. Scanned Educational certificates (SSC, Intermediate, provisional or convocation of highest degree),Also fill your Educational Detail in Verity Portal	<br>							
                                                5. Scanned photograph<br>								
                                                6. Your current and permanent address details. ,Also fill Contact Tab in Verity Portal<br>								
                                                Send your acceptance for the offer along with the documents. Your date of joining will be informed to you soon. <br></br>								
                                                <b>Note: In case you have experience, please upload 3 months salary payslips,6 months bank statements and all your employeement related documents,And also fill experience tab in Verity Portal</b><br>";
                                   $text .="Verity Portal :".$URL." <br>Your user credentials are :<br>"
                                           . "<b>User Name </b>".$cand_data[emailid] ." <br><b>password</b> ".$password;
                                }
        ...
        public function createcandidateuser($cand_data , $requisition_data , $report_person_data)
        {
		...
		$candidate_code = isset($identity_codes[0])?$identity_codes[0]['cand_code']:"";
                $employeeNumId = '';
