...
class Default_DashboardController extends Zend_Controller_Action{
	...
	{		
		...
		$usermodel = new Default_Model_Users();
		$emppersonal = new Default_Model_Emppersonaldetails();
		$userlogmodel = new Default_Model_Userloginlog();
		$empsummerymodel = new Default_Model_Employee();

      //  print_r($empsummery);exit;
		$userlog = $userlogmodel->getUserLastlogin($id);

		$dob = $emppersonal->getsingleEmpPerDetailsData($id);
		if($dob[0][dob]!='')
        {
            $dob =$dob[0]['dob'];
            $empsummery = $empsummerymodel->getEmp_from_summary($id);
            $jobtitle =$empsummery['jobtitle_name'];
            $position =$empsummery['position_name'];
            $employeeId =$empsummery['employeeId'];
        ...
        if($id == SUPERADMIN)
        {
            ...
            $location = $getuserdetails[0]['location'];
            $aboutme = $getuserdetails[0]['aboutme'];
            $logindatetime = $userlog[0]['logindatetime'];
            // $adusername = $getuserdetails[0]['adusername'];
            $profileimage = $getuserdetails[0]['profileimg'];
            $registerdate = $getuserdetails[0]['createddate'];
            //print_r($getuserdetails[0]);exit;
        ...
        $this->view->logindatetime = $logindatetime;
        $this->view->registerdate = $registerdate;
	...
	public function saveProfileDetails($viewprofileform)
	{
			...
			if($viewprofileform->isValid($this->_request->getPost())){
                ...
                $aboutme = $this->_request->getParam('aboutme');
                ...
                if($id!='') {
                    ...
                    $update_profile = $usersModel->updateuserprofile($data, $where);
                }
