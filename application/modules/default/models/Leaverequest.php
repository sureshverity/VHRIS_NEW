...
class Default_Model_Leaverequest extends Zend_Db_Table_Abstract
{
   ...
   public function getAvailableLeavesbyleavetype($loginUserId,$leavetype)
        {
            //echo $leavetype; 
            $db = Zend_Db_Table::getDefaultAdapter();
            $query = "SELECT `e`.`emp_leave_limit` AS `leavelimit`, e.emp_leave_limit - e.used_leaves AS `remainingleaves
                        `,sum(ml.appliedleavescount) as appliedleavescount,mlt.numberofdays FROM `main_employeeleaves` AS `e` inner join main_leaverequest ml on e.user_id=ml.user_id
                        inner join main_employeeleavetypes mlt on ml.leavetypeid=mlt.id WHERE (e.user_id=$loginUserId AND e.alloted_year = now() AND e.isactive =
                         1 and ml.leavetypeid=$leavetype)"; 
            $result = $db->query($query)->fetchAll();
           // print_r($result); exit;
	    return $result;
        }
		...
		if($objName == 'manageremployeevacations')
		{
		       if($searchData != '' && $searchData!='undefined')
				...
				$tableFields = array('action'=>'Action','userfullname' => 'Employee','leavetype' => 'Leave Type',
                    'from_date' => 'From','to_date' => 'To','appliedleavescount' => 'Days','applieddate' => 'Applied On','leavestatus' => 'Status');
