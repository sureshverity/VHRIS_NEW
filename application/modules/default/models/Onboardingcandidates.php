...
class Default_Model_Onboardingcandidates extends Zend_Db_Table_Abstract
{
	...
	public function getEmpScreeningData($sort, $by, $pageNo, $perPage,$searchQuery,$filter='1')
	{
		...
		$db = Zend_Db_Table::getDefaultAdapter();
			...
			$where = "me.backgroundchk_status <> 'Not Applicable' AND me.backgroundchk_status <> 'Yet to start' ";
        ...
        public function onboardingcandidates()
        {
            $db = Zend_Db_Table::getDefaultAdapter();
		
			//$where = "me.backgroundchk_status <> 'Not Applicable' AND me.backgroundchk_status <> 'Yet to start' AND me.cand_status <>'Recruited'";
			$where ="me.cand_status ='Selected'";
                         $empScreeningData = $this->select()
						->setIntegrityCheck(false)
						->from(array('me' => 'main_candidatedetails'),array('id'=>'distinct concat(me.id,"-2")','backgroundchk_status'=>'if(me.backgroundchk_status="Completed","Complete",me.backgroundchk_status)','candidate_name'=>'me.candidate_name','isactive'=>'if(me.isactive = 1, "Active","Inactive")','createddate'=>'me.createddate','cand_location'=>'me.cand_location','requisition_id'=>'me.requisition_id','emailid'=>'me.emailid'))
...
//						->limitPage($pageNo, $perPage);	
		...
		//return $empScreeningData; 
                return $this->fetchAll($empScreeningData)->toArray();
        }
