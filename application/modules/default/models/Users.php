...
class Default_Model_Users extends Zend_Db_Table_Abstract
{
    ...
    public function updateuserprofile($data,$where,$userid = '')
    {
        if ($where != '') {
            $this->update($data, $where);
            if ($userid) {
                $db = Zend_Db_Table::getDefaultAdapter();
              echo  $query = "update main_users  set firstname = '" . $data['firstname'] . "',lastname = '" . $data['lastname'] . "',location = '" . $data['location'] . "',aboutme = '" . $data['aboutme'] . "', modifiedby = " . $data['modifiedby'] . ", modifieddate = '" . $data['modifieddate'] . "' where user_id = " . $userid;exit;
                $db->query($query);
            }
            return 'update';
        }
	...
	public function getEmpDetailsByEmailAddress($email)
	{
		 $select = $this->select()
		->setIntegrityCheck(false)
		->from(array('u'=>'main_users'), array('u.id','u.emppassword','u.userfullname','u.isactive','u.emptemplock'))
	...
	{
           // echo $dataTmp['logindatetime']; exit;
		$db = Zend_Db_Table::getDefaultAdapter();
 ...
 ELSE CASE WHEN u.id IN (SELECT reporting_manager FROM main_employees_summary) THEN 'Manager'
      ELSE 'Employee' END END)")))
       ...
       public function getUseridbyreqid($req_id)
        {
            $select = $this->select()
		->setIntegrityCheck(false)
		->from(array('u'=>'main_users'), array('id'=>'id'))
		->where('u.rccandidatename="'.$req_id.'" AND u.isactive = 1 AND u.emptemplock = 0');
