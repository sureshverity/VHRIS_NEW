...
class Default_Model_Widgets extends Zend_Db_Table_Abstract
{
	...
	public function getTodaysBirthdays($businessunit_id,$department_id,$isOrganizationHead='')
	{
		...
		$qryStr = "select id,firstname, lastname,profileimg from main_users where id in (Select p.user_id from main_emppersonaldetails p inner join main_employees e on p.user_id = e.user_id  where day(now()) = day(p.dob) and month(now()) = month(p.dob) ".$whereStr.")";
	...
	public function getUpcomingBirthdays($businessunit_id,$department_id,$isOrganizationHead='')
	{
		...
		* @var String $qryStr
		*/
		$qryStr = 'SELECT e.userfullname, p.dob, abs(if(right(curdate(),5) >= right(p.dob,5),
                    datediff(curdate(),concat(year(curdate()+ interval 1 year),right(p.dob,6))) ,
                    datediff(concat(year(curdate()),right(p.dob,6)),curdate()))) as days FROM  main_emppersonaldetails p  
