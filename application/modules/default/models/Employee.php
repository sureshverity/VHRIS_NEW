	...
	*/
	...
	$controllerName = $request->getRequest()->getControllerName();
        ...
        if($controllerName=='employee' && ($loginUserRole == SUPERADMINROLE || $loginUserGroup == HR_GROUP || $loginUserGroup == MANAGEMENT_GROUP))                            
        	$where = "e.emp_status_id != 11  AND e.isactive != 5 AND e.user_id != ".$loginUserId." ";
        elseif($loginUserRole == 7)
            $where = "e.emp_status_id = 11  AND e.isactive != 5 AND e.user_id != ".$loginUserId." ";
       // elseif($emprole == 14)
         //    $where = "e.emp_status_id = 11 AND e.emprole = 14 AND e.isactive != 5 AND e.user_id != ".$loginUserId." ";
        else	  
        	$where = "e.emp_status_id != 11 AND e.isactive = 1 AND e.user_id != ".$loginUserId." ";
...
//        $empData = $db->query("SELECT e.*,u.*,p.prefix,p.isactive as active_prefix FROM main_employees e
//                INNER JOIN main_users u ON e.user_id = u.id left JOIN main_prefix p ON e.prefix_id = p.id
//                           WHERE e.emp_status_id != 11 AND e.isactive = 1 AND u.isactive =1  AND u.userstatus ='old'");
        
        $empData = $db->query("SELECT  e.* from main_employees_summary e where e.emprole !=14 AND e.isactive= 1 ORDER by e.userfullname");
	...
	public function getIndividualEmpDetails($userid)
	{
        ...
        public function getEmp_from_summaryByEmployeeId($employeeid)
        {
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = "select e.* from main_employees_summary e where e.id = ".$employeeid." ";
            $res = $db->query($qry)->fetch();
            return $res;		
	}
