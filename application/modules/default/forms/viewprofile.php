...
class Default_Form_viewprofile extends Zend_Form
{
    public function init()
    {
        ...
        $location->setAttrib('length', 70);
        

        $aboutme = new Zend_Form_Element_TextArea("aboutme");
        $aboutme->setLabel("About Me");	
        $aboutme->setAttrib("class", "form-control");
        $aboutme->setAttrib('rows', 5);
                   
                        
			
        $emailaddress = new Zend_Form_Element_Text('emailaddress');
        $emailaddress->setLabel("Email");
        $emailaddress->setAttrib("readonly",true);
        $emailaddress->setAttrib("class", "form-control");

        $employeeid = new Zend_Form_Element_Text('employeeId');
        $employeeid->setLabel("#ID");
        $employeeid->setAttrib("readonly",true);
        $employeeid->setAttrib("class", "form-control");
				
        $submit = new Zend_Form_Element_Submit('submit');
        $submit->setAttrib('id', 'submitbutton');
        $submit->setAttrib('class', 'btn btn-primary btn-rounded pull-right');
        $submit->setLabel('Save');
		
        $this->addElements(array($id,$firstname,$lastname,$emailaddress,$location,$employeeid,$aboutme,$submit));
