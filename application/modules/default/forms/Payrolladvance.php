...
class Default_Form_Payrolladvance extends Zend_Form
{
	public function init()
	{
		$this->setMethod('post');
		
		$this->setAttrib('action',BASE_URL.'payrollemployeeadvances/edit');
		
		$this->setAttrib('id', 'formid');
		$this->setAttrib('name', 'payrolladvance');
		...
		$currency->setLabel("Amount");
                $currency->setAttrib('class', 'sCurrency');
        ...
        $amount->setAttrib('class', 'sAmount');
		...
		$amount->addValidator('NotEmpty', false, array('messages' => 'Please enter amount.'));
         ...
         $emi_months = new Zend_Form_Element_Select('emi_months');
		$emi_months->setLabel('EMI');	
        $emi_months->setAttrib('class', 'selectoption');
        $emi_months->setRegisterInArrayValidator(false);
		$emi_months->addMultiOption('','Select EMI In Months');
		$emi_months->setRequired(FALSE);
		...
		$this->addElements(array($id,$employeess,$currency,$amount,$emi_months,$payment_mode,$paymentref,$description,$submit));
