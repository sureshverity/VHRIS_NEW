...
class Zend_View_Helper_Menubuilder extends Zend_View_Helper_Abstract {
	...
	**/
	...
	{        			
        ...
        $parent_menu_icon_class = array(
                                                    SITECONFIGURATION => 'fa icon_lg icon-siteconfig',
                                                    ORGANIZATION => "fa icon_lg icon-organization",
                                                    HUMANRESOURCE => "fa icon_lg icon-hr",
                                                    RESOURCEREQUISITION => "fa icon_lg icon-talent",
                                                    EMPLOYEESELFSERVICE => "fa icon_lg icon-selfservice",
                                                    BGCHECKS => "fa icon_lg icon-background",
                                                    REPORTS => "fa icon_lg icon-analytics",
                                                    MANAGEMODULE => "fa icon_lg icon-module",
                                                    SERVICEDESK => 'fa icon_lg icon-servicerequest',
                                                    PERFORMANCEAPPRAISAL => "fa icon_lg icon-appraisal",
                                                    ATTENDANCE => 'fa icon_lg icon-attendance',
                                                    DASHBOARD_MENU => "fa icon_lg icon-dashboard",
                                                    PAYROLL => "fa icon_lg icon-payroll",
                                                    TIMEMANAGEMENT => "fa icon_lg icon-time",
                                                    EXPENSES => "fa icon_lg icon-expense",
                                                    ASSETS =>"fa icon_lg icon-assests",
                                                    LOGS =>"fa icon_lg icon-logs"   
			);
		...
		{           $childs_menu = "";
                    ...
                    if(isset($data['childs']) && count($data['childs']) > 0)
                    {
                        ...
                        foreach($data['childs'] as $ch_menu_id => $ch_menu_data)
                        {
                            ...
                            {
                                $childs_menu .= "<li  class='xn-openable'><a id='acc_li_toggle_".$ch_menu_data['id']."' href='#'>".$ch_menu_data['menuName']."</a>";
                                $childs_menu .= "<ul>";
                                foreach($ch_menu_data['childs'] as $ch2_menu_id => $ch2_menu_data)
                                {
                                    ...
                                    {
                                        $childs_menu .= "<li menu-url ='".rtrim(BASE_URL,"/").$ch2_menu_data['url']."'><a id='".$ch2_menu_data['id']."' href='".rtrim(BASE_URL,"/").$ch2_menu_data['url']."'>".$ch2_menu_data['menuName']."</a></li>";
                                    }
                                }
                                $childs_menu .= "</ul>";
                            }
                            ...
                            {
                            $childs_menu .= "<li menu-url ='".rtrim(BASE_URL,"/").$ch_menu_data['url']."'><a id='".$ch_menu_data['id']."' href='".rtrim(BASE_URL,"/").$ch_menu_data['url']."'>".$ch_menu_data['menuName']."</a>";
                            }
                            $childs_menu .="</li>";
                        }
                        $childs_menu .= "</ul>";
                    ...
                    else 
                    {
                        ...
                        if($menuid != TIMEMANAGEMENT)
                        {
                            ...
                            if($isopenable == "")
                            {
                                $parent_menu .= "<li menu-url ='".$parent_url."' id='main_parent_".$menuid."' ".$dummy_parent_div." ".$super_str." super-parent = 'main_parent_".$menuid."' class='clickable_menu ".$isopenable.$parent_menu_selected_class[$menuid].($groupbasedclass!=''?"-main-common ":"-main ").$tour_menu_class[$menuid]."' ><a id='".$menuid."' href='".$parent_url."'><span class='".$parent_menu_icon_class[$menuid]."'></span><span class='xn-text'>".$data['menuName']."</span></a></li>";  
                            }
                            else
                            {
                            $parent_menu .= "<li id='main_parent_".$menuid."' ".$dummy_parent_div." ".$super_str." super-parent = 'main_parent_".$menuid."' class='clickable_menu ".$isopenable.$parent_menu_selected_class[$menuid].($groupbasedclass!=''?"-main-common ":"-main ").$tour_menu_class[$menuid]."' ><a id='".$menuid."' href='javascript:void(0);'><span class='".$parent_menu_icon_class[$menuid]."'></span><span class='xn-text'>".$data['menuName']."</span></a>".$childs_menu."</li>";  
                            }
                        }
                    }
		...
		{
			$parent_menu .= "<li menu-url ='".BASE_URL."timemanagement' id='main_parent_".TIMEMANAGEMENT."' super-parent = 'main_parent_".TIMEMANAGEMENT."' class = '".$clas_drag."clickable_menu ".$parent_menu_selected_class[TIMEMANAGEMENT]."-main ".$tour_menu_class[TIMEMANAGEMENT]."'  selected-class = '".$parent_menu_selected_class[TIMEMANAGEMENT]."' > <a id='".TIMEMANAGEMENT."' href ='".BASE_URL."timemanagement' ><span class='".$parent_menu_icon_class[TIMEMANAGEMENT]."'></span><span class='xn-text'>".$act_menus[TIMEMANAGEMENT]['menuName']."</span></a></li>";
		}
        if(($userRole == SUPERADMIN || $usergroup == MANAGEMENT_GROUP) && $con != 'settings')
		{		
             
			
             $parent_menu .= "<li id='main_parent_logs' super-parent = 'main_parent_".TIMEMANAGEMENT."' class = 'clickable_menu log-selected-main tour_logs xn-openable'  > <a id='logs' href ='#' ><span class='fa icon_lg icon-logs'></span><span class='xn-text'>Logs</span></a>";
             $parent_menu  .="<ul>";
             $parent_menu  .="<li ><a href='".BASE_URL."logmanager'>Activity Log</a></li>";
             $parent_menu  .="<li ><a href='".BASE_URL."userloginlog'>User Log</a></li>";
             $parent_menu  .="</ul>";
             $parent_menu  .="</li>";
		}
